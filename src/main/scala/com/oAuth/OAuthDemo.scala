  ...
  def routes(config: AppConfig): HttpRoutes[IO] = HttpRoutes.of[IO] {
    case req @ GET -> Root / "home" =>
      StaticFile
        ...
        .getOrElseF(NotFound())
    case GET -> Root / "callback" :? GithubTokenQueryParamMatcher(
          code
        ) => // client ID and secret loaded from appConfig.json
      getGithubOAuthResult(code, config.credentials("github")).flatMap(Ok(_))

    case GET -> Root / "googlecallback" :? GoogleTokenQueryParamMatcher(
          code
        ) => // client ID and secret loaded from appConfig.json
      getGoogleOAuthResult(code, config.credentials("google")).flatMap(Ok(_))

    case req @ GET -> Root / "orycallback" =>
      req.cookies
        .find(_.name == "ory_kratos_session")
        .map { (cookie: RequestCookie) => 
          Ok(getOryOAuthResult(cookie).handleError {
            case OryAuth.Unauthorized => s"Invalid session cookie"
          })
        }
